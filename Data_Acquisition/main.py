...
# Custom imports
from functions import list_gen, save_labels, folder_gen, name_gen, net_streams, saver, zipr, imp_check, exp_params, generic_pres, imp_check_pauser
...
if __name__ == '__main__':
    ...
    aug_duration = 0.05  # 0.1  # Duration of the augmentation on screen
    aug_wait = 0.1  # 0.5  # Temporal distance between augmentations
    inter_trial_int = 0.5  # 0.5
    inter_seq_interval = 1  # 0.5
    cue_interval = 0.5  # 0.5
    ...
    data_direc = './Data/P_3Data//'+emoji_count
    lab_direc = './Data/P_3Data//'+emoji_count+'/Labels/'
    folder_gen(data_direc, lab_direc)
    ...
    for t in range(estimulus.num_trials):
        ...
        for s in range(estimulus.num_seq):
            ...
            'Impedance Checking and Pause Activation.'
            # Impedances Check for range over sequence for Cz Electrode.
            imp_check(imp, Cz=1, limit=100, plotter=1, verbose=1)
            ...
            saver(filename, eeg, eeg_time, data_direc)
            filename = 'impedanc_t{2}{0}_s{1}_'.format(t + 1, s + 1, namer)
            saver(filename, imp, imp_time, data_direc)
        'Trial Prediction'
    ...
    # Marker Check
    marker_file = './Data/P_3Data/'+emoji_count+'/marker_data.npz'
